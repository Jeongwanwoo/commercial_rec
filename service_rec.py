...
# 상주_유동비율 데이터 로드(merge_set_flo_prop2.csv)
data_pop = pd.read_csv("사용자 경로 입력/merge_set_flo_prop2.csv")
# 매출 데이터 로드(annual_sales_new.csv)
data_sales = pd.read_csv("사용자 경로 입력/annual_sales_new.csv")
# 상권-업종 별 분기매출 평균(point_service_sales.csv)
service_sales = pd.read_csv("사용자 경로 입력/point_service_sales.csv", encoding='utf-8', index_col=False)
